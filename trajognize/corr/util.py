...
def matrix2corrline(data):
    ...
    return "\t".join([data[0][0]] + [data[data[0].index(sx)][data[0].index(sy)] \
            for sx in headers for sy in headers])
...
def pairparams2params(headerline, corrline):
    ...
    normDSline = '\t'.join(["%s_normDS" % name] + ["%g" % normDS[strid] for strid in strids])
    ...
    ROWSUMline = '\t'.join(["%s_rowsum" % name] + ["%g" % ROWSUM[strid] for strid in strids])

    WINAA = trajognize.calc.hierarchy.win_above_average(datadict)
    WINAAline = '\t'.join(["%s_winaboveavg" % name] + ["%g" % WINAA[strid] for strid in strids])

    LOSEAA = trajognize.calc.hierarchy.lose_above_average(datadict)
    LOSEAAline = '\t'.join(["%s_loseaboveavg" % name] + ["%g" % LOSEAA[strid] for strid in strids])

    # write to output format
    paramsheaderline = strids2headerline(strids, False)
    paramscorrlines = '\n'.join([normDSline, BBSline, LDIline, ROWSUMline, WINAAline, LOSEAAline])
