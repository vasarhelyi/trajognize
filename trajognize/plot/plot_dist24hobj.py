...
def get_gnuplot_script(
    ...
    if weekday in project_settings.weekly_feeding_times:
        for (start, duration) in project_settings.weekly_feeding_times[weekday]:
            data["feedingrect"] += GNUPLOT_FEEDINGRECT_TEMPLATE % (
                start,
                start + duration,
            )
...
def main(argv=[]):
    ...
    for inputfile in inputfiles:
        ...
        for index in range(len(headers)):
            ...
            spgm.create_picture_description(
                outputfiles[0],
                [name, "individual data 00-24h", exp],
                inputfile,
                gnufile,
            )
            spgm.create_picture_description(
                outputfiles[1],
                [name, "individual data 05-07h", exp],
                inputfile,
                gnufile,
            )
            spgm.create_picture_description(
                outputfiles[2],
                [name, "individual data 11-13h", exp],
                inputfile,
                gnufile,
            )
            spgm.create_picture_description(
                outputfiles[3],
                [name, "individual data 17-19h", exp],
                inputfile,
                gnufile,
            )
            spgm.create_picture_description(
    ...
    spgm.create_gallery_description(
        head, "24h distribution of barcode occurrences around objects"
    )
    spgm.create_gallery_description(
        os.path.join(head, plotdir),
        """Plotted results for 24h barcode distribution around objects.
