"""This script generates plots for 'nearestneighbor' type trajognize.stat outputs.
...
def get_categories_from_name(name):
    """Get light, realvirtany and group from paragraph header (name), e.g.:

    nearestneighbor_daylight_real_group_G3S_C
    nearestneighbor_daylight_virtual_group_G3S
    nearestneighbor_daylight_any_F
...
def main(argv=[]):
    ...
    for inputfile in inputfiles:
        ...
        for index in range(len(alldata)):
            ...
            outdir = os.path.join(head, plotdir, exp, group, light, realvirtany)
