...
where inputfile(s) is/are the output of trajognize.stat ButtHead object (.txt)
...
def main(argv=[]):
    ...
    spgm.create_gallery_description(
        head, "Butthead matrices reordered with the Eades-heuristics"
    )
    spgm.create_gallery_description(
        os.path.join(head, plotdir),
        """Plotted results for butthead pairwise matrices.
