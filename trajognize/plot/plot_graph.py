...
def remove_negligible_data(data):
    ...
    if removed:
        print(
            ...
            "elements -",
            removed,
            "- removed from matrix for igraph plot, "
...
def main(argv=[]):
    ...
    if not create_ncol_file(data, inputfilencol):
        return
    # get other variables
    ...
    lo, extd_graph = g.layout_sugiyama(
        weights=g.es["weight"], return_extended_graph=True
    )
    ...
    extd_graph.vs["width"] = extd_graph.vs["size"] = [
        extd_graph.vs["label_size"][i] * len(extd_graph.vs[i]["label"])
    ...
    if wmax != wmin:
        normalized_weights = [
            5 * (x - wmin) / (wmax - wmin) + 5 for x in normalized_weights
        ]  # 5-10
