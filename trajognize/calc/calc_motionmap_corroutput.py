...
def get_categories_from_filename(filename):
    ...
    stat_motionmap.ROG__exp_fifth_G1_G4_large_G2_G3_small.zip

    """
    match = re.match(r'^stat_motionmap\.(.*)__exp_(.*)\.zip$', filename)
    if match:
        strid = match.group(1)
        exp = match.group(2)
        return (strid, exp)
...
def main(argv=[]):
    ...
    for inputfile in inputfiles:
        ...
        motionmaps = trajognize.stat.init.motionmap_t()
        ...
        days = trajognize.stat.experiments.get_days_since_start(exps[exp], exps[exp]['stop']) + 1
    ...
    for exp in database:
        ...
        for group in exps[exp]['groups'].keys() + ['all']:
            ...
            for key in keys:
                corrline = "\t".join(["motionmap_%s" % ("_".join(key))] +
