...
argparser.add_argument(
    ...
    action="store_true",
    default=False,
    help="do not calculate distribution of color blobs",
)
argparser.add_argument(
    "-nm",
    "--nomotiondist",
    ...
    action="store_true",
    default=False,
    help="do not calculate distribution of motion blobs",
)
argparser.add_argument(
    "-nh",
    "--noheatmap",
...
argparser.add_argument(
    ...
    required=True,
    dest="inputpath",
    help="define individual blob input file, or a path that has blob files at [PATH]*/OUT/*.blobs",
    metavar="PATH",
)
argparser.add_argument(
    "-p",
        ...
        [
            [
                [
                    0
                    for y in range(
                        int(project_settings.image_size.y / heatmap_bin_size)
                    )
                ]
...
for inputfile in files:
    try:
        ...
        if not args.noheatmap:
            ...
            if not light_log or not cage_log:
                continue
            ...
            try:
                ...
                for currentframe in range(len(color_blobs)):
                    for blob in color_blobs[currentframe]:
                        ...
                        good += 1
                print(
                    "  %d blobs added with 10*axisA/axisB <= %d" % (good, max_sizedist)
                )
                sys.stdout.flush()
                ...
                for currentframe in range(len(color_blobs)):
                    for blob in color_blobs[currentframe]:
                        d = int(
                            max_sizedist
                            * sqrt(blob.axisA * blob.axisA - blob.axisB * blob.axisB)
                            / blob.axisA
                        )
                        if d > max_sizedist:
                            continue
            ...
            for currentframe in range(len(color_blobs)):
                for i in range(len(color_blobs[currentframe])):
                    for j in range(i):
                        ...
                        d = int(
                            trajognize.algo.get_distance(
                                color_blobs[currentframe][i],
                                color_blobs[currentframe][j],
                            )
                        )
                        if d > max_sdist:
                            continue
                        # same color
                        if (
                            color_blobs[currentframe][i].color
                            == color_blobs[currentframe][j].color
                        ):
                            ...
                            good += 1
            print(
                "  %d blob pairs added with diff color and distance <= %d on the same frame"
                % (good, max_sdist)
            )
            print(
                "  %d blob pairs added with same color and distance <= %d on the same frame"
                % (good_samecolor, max_sdist)
            )
            sys.stdout.flush()
            ...
            for currentframe in range(1, len(color_blobs)):
                for i in range(len(color_blobs[currentframe])):
                    for j in range(len(color_blobs[currentframe - 1])):
                        ...
                        if (
                            color_blobs[currentframe][i].color
                            == color_blobs[currentframe - 1][j].color
                        ):
                            tdists[d] += 1
                            good += 1
                        else:
                            ...
                            good_diffcolor += 1
            print(
                "  %d blob pairs added with same color and distance <= %d on consecutive frames"
                % (good, max_tdist)
            )
            print(
                "  %d blob pairs added with diff color and distance <= %d on consecutive frames"
                % (good_diffcolor, max_tdist)
            )
            sys.stdout.flush()
            ...
            for currentframe in range(len(md_blobs)):
                for i in range(len(md_blobs[currentframe])):
                    for j in range(i):
                        ...
                        d = int(
                            trajognize.algo.get_distance(
                                md_blobs[currentframe][i], md_blobs[currentframe][j]
                            )
                        )
                        if d > max_sdist:
                            continue
                        ...
                        good += 1
            print(
                "  %d blob pairs added with distance <= %d on the same frame"
                % (good, max_sdist)
            )
            sys.stdout.flush()
                ...
                for i in range(len(md_blobs[currentframe])):
                    for j in range(len(md_blobs[currentframe - 1])):
                        ...
                        (
                            delta_o_avg_md[d],
                            delta_o_std_md[d],
                        ) = trajognize.algo.calculate_running_avg(
                            acos(abs(cos(blobi.orientation - blobj.orientation)))
                            * 180
                            / pi,
                            tdists_md[d],
                            delta_o_avg_md[d],
                        ...
                        good += 1
            print(
                "  %d blob pairs added with distance <= %d on consecutive frames"
                % (good, max_tdist)
            )
            sys.stdout.flush()
        ...
        if not args.noheatmap:
            ...
            for currentframe in range(len(color_blobs)):
                ...
                for i in range(len(color_blobs[currentframe])):
                    ...
                    heatmaps[light][color_blobs[currentframe][i].color][
                        int(centerx / heatmap_bin_size)
                    ][int(centery / heatmap_bin_size)] += 1
                    good[light] += 1
            ...
            for currentframe in range(len(md_blobs)):
                ...
                for i in range(len(md_blobs[currentframe])):
                    ...
                    if (
                        ...
                        continue
                    # store good ones on heatmap
                    heatmap_md[light][int(centerx / heatmap_bin_size)][
            ...
            for currentframe in range(len(rat_blobs)):
                ...
                for i in range(len(rat_blobs[currentframe])):
                    ...
                    if (
                        ...
                        continue
                    # store good ones on heatmap
                    heatmap_rat[light][int(centerx / heatmap_bin_size)][
                        int(centery / heatmap_bin_size)
                    ] += 1
                    good[light] += 1
    ...
    for i in range(max_tdist + 1):
        print(
            ...
            % (
                ...
                tdists_md[i],
                delta_o_avg_md[i],
                sqrt(delta_o_std_md[i] / tdists_md[i])
                if tdists_md[i]
                else float("nan"),
            )
        )
...
if not args.noheatmap:
    for light in range(len(project_settings.good_light)):
        for k in range(5):
            print(
                "\n\n# heatmap of %s %s blobs"
                % (project_settings.good_light[light], project_settings.color_names[k])
            )
            print(
                "heatmap_%s_%s"
                % (project_settings.good_light[light], project_settings.color_names[k]),
